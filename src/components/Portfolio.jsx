...
const Portfolio = () => {
  ...
  return (
    <div className="container mx-auto p-10 md:p-20 md:px-[8%] min-h-[100vh] flex flex-col items-center justify-center">
      ...
      {/* Modal */}
      <div id="hs-slide-down-animation-modal" className={showModal ? 'hs-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/80' : 'hidden'}>
        <div className="md:flex max-w-[90%] h-auto md:h-auto md:max-h-[80%] bg-neutral-800 overflow-y-auto top-0 rounded-lg py-4 px-8 border-2 border-neutral-900 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 opacity-0 ease-out transition-all">
          {/* Modal content */}
      ...
      <div className="md:container md:mx-auto bg-neutral-900 grid grid-cols-2 grid-cols-2 md:grid-cols-4 gap-6">
        {works.map((item) => (
          <div key={item.id}>
            <div className="cursor-pointer relative max-w-xs overflow-hidden bg-cover bg-no-repeat" onClick={() => openModal(item)} >
