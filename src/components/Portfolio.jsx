  ...
  return (
    <div className="container mx-auto p-10 md:p-20 md:px-[8%] min-h-[100vh] flex flex-col items-center justify-center">
        ...
        <div className={model ? 'fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80' : 'hidden'}>
            <div className="md:flex max-w-[80%] max-h-[80%] md:max-h-[80%] dark:bg-neutral-800 rounded-lg p-10 border-2 dark:border-neutral-900 transition">
                <div className="modal-image flex flex-col justify-center items-center md:w-[60%]">
                    <h2 className='font-bold text-amber-100 text-lg md:text-2xl  md:hidden'>{tempTitle}</h2>
                    <img src={tempimgSrc} alt={tempDesc} className="rounded h-full w-full drop-shadow-xl aspect-square md:aspect-square object-cover md:object-contain h-full" />
                </div>
                <div className="flex flex-col modal-details md:ml-4 w-full text-amber-50 md:text-left  md:pt-0 md:w-[40%]">
                    ...
                    <h2 className="md:text-xl text-sm text-left md:text-center sm:hidden block overflow-y-auto max-h-24 mb-4">{tempDesc}</h2>
                </div>
