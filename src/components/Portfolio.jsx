  ...
  return (
    <div className="p-10 md:p-20 md:px-80 min-h-[100vh] flex flex-col items-center justify-center">
        ...
        <div className={model ? 'fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80' : 'hidden'}>
            <div className="md:flex max-w-[80%] max-h-[100%] md:max-h-[80%] dark:bg-neutral-800 rounded-lg p-10 border-2 dark:border-neutral-900">
                <div className="modal-image flex flex-col justify-center items-center md:w-[60%]">
                    ...
                    <img src={tempimgSrc} alt={tempDesc} className="rounded drop-shadow-xl aspect-square md:aspect-auto object-cover md:object-contain h-full" />
                </div>
                <div className="flex flex-col modal-details md:ml-4 w-full text-amber-50 md:text-left pt-4 md:pt-0 md:w-[40%]">
                    <h2 className='font-bold text-2xl mb-2 text-amber-100 hidden md:block'>{tempTitle}</h2>
                    <h2 className="text-base text-left text-left hidden text-amber-50 md:block overflow-y-auto">{tempDesc}</h2>
                    <a href={tempLink} className="my-4 ">
                        <button className="bg-amber-500 md:w-40 w-full hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow-lg">Visit Link</button>
                    </a>
                    <h2 className="md:text-xl text-sm text-left md:text-center sm:hidden overflow-y-auto max-h-56 mb-4">{tempDesc}</h2>
                </div>
            ...
            {works.map((item) => (
                <div key={item.id}>
                    <div className="cursor-pointer relative max-w-xs overflow-hidden bg-cover bg-no-repeat" onClick={() => getAll(item)} >
                        ...
                        <div class="absolute bottom-0 left-0 right-0 top-0 h-full w-full overflow-hidden bg-neutral-900 bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-60"></div>
                    </div>
                </div>
            ))}
        </div>
